...
export async function GET(req:NextRequest){
 ...
 const [health,subs,claims,feedback,analytics,cache,registry]=await Promise.all([
 ...
 return NextResponse.json({source_health:health.data||[],source_registry:registry.data||[],pending_submissions:subs.data||[],organization_claims:claims.data||[],feedback:feedback.data||[],analytics:counts,active_cached_listings:cache.count||0,generated_at:new Date().toISOString()});
}
export async function POST(req:NextRequest){
 ...
 const url=process.env.NEXT_PUBLIC_SUPABASE_URL||"https://pixlqytdhgxcfgagwijy.supabase.co",key=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!key)return NextResponse.json({error:"server config"},{status:500});
 try{const b=await req.json(),id=String(b.id||"").toLowerCase().replace(/[^a-z0-9-]/g,"").slice(0,80),name=String(b.name||"").slice(0,160),source_type=String(b.source_type||""),sourceUrl=new URL(String(b.url||"")),latitude=Number(b.latitude),longitude=Number(b.longitude),coverage_miles=Math.max(1,Math.min(250,Number(b.coverage_miles||50)));if(!id||!name||!["localist","ucf_json","tribe"].includes(source_type)||!Number.isFinite(latitude)||!Number.isFinite(longitude)||sourceUrl.protocol!=="https:")return NextResponse.json({error:"invalid source"},{status:400});const db=createClient(url,key);const{error}=await db.from("source_registry").upsert({id,name,source_type,url:sourceUrl.toString(),latitude,longitude,coverage_miles,trust_tier:"official",free_policy:"explicit_only",active:true,last_verified_at:null,updated_at:new Date().toISOString()},{onConflict:"id"});if(error)return NextResponse.json({error:error.message},{status:500});return NextResponse.json({ok:true,id})}catch{return NextResponse.json({error:"bad request"},{status:400})}
}