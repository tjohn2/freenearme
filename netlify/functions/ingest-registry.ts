...
async function load(src:any){
 try{
  ...
  if(src.source_type==="umich_json"){
   const r=await fetch(src.url,{headers:{"User-Agent":"FreeNearMe/1.0 registry ingester"}});if(!r.ok)return[];const rows=await r.json();
   return(rows||[]).filter((e:any)=>explicitFree(e.cost,e.description,e.event_title,e.title,e.tags)||eligibleFree(e.description,e.event_title,e.title,e.tags)).map((e:any)=>{const txt=[e.event_title||e.title,e.description,e.tags,e.event_type].map(clean).join(" "),elig=eligibleFree(txt),lat=Number(e.latitude??e.lat),lng=Number(e.longitude??e.lng);return{id:"umich:"+e.id,title:clean(e.event_title||e.title),description:clean(e.description).slice(0,700),category:classify(txt),venue:clean(e.location_name||e.location||src.name),address:clean(e.address||""),latitude:lat,longitude:lng,starts_at:e.datetime_start||e.start_date||null,ends_at:e.datetime_end||e.end_date||null,free_type:elig?"eligible":"free",requirements:elig?"Free only for the eligible audience named by the organizer. Check registration and access rules.":"Official university feed explicitly describes this event as free. Check registration and access rules.",source_url:e.permalink||e.url||"https://events.umich.edu/",source_name:src.name,image_url:e.image_url||e.image,verification_status:"source-verified",offer_kind:"event",source_tier:"official"}});
  }
