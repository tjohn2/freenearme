...
export default async()=>{
 ...
 await db.from("job_runs").insert({job_name:"dedupe-cache",status:"ok",processed:removed,finished_at:new Date().toISOString()});return new Response(JSON.stringify({ok:true,removed}),{headers:{"content-type":"application/json"}});
};
